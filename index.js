...
if (!specific) return res.status(404).json({ message: "Post not found" });
res.json(specific);
...
app.patch("/post/edit/:id", (req, res) => {
  ...
  const currentPost = posts[searchIndex];

  const modified = {
    ...currentPost,                // keep existing fields
